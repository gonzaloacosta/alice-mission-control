    ...
    <div style={{ padding: '20px 24px', height: '100%', overflowY: 'auto' }}>
      ...
      <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {FILTERS.map((filter) => {
          ...
          const color = filter === 'All' ? '#c6d4ef' : ASSIGNEE_COLOR[filter];
          return (
            <button
              key={filter}
              type="button"
      ...
      <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 8, color: '#8fa5c7', fontSize: 11, fontFamily: 'Share Tech Mono, monospace' }}>
        <span style={{ border: '1px solid rgba(255,255,255,0.12)', borderRadius: 999, padding: '4px 8px', background: 'rgba(8,12,28,0.8)' }}>
        ...
        {COLUMNS.map((col) => (
          <div key={col.key} style={{ background: 'rgba(8,12,28,0.9)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ color: '#00f0ff', fontFamily: 'Orbitron, sans-serif', fontSize: 12 }}>{col.label}</div>
              <div style={{ color: '#9fb6db', fontFamily: 'Share Tech Mono, monospace', fontSize: 11 }}>{totalByStatus[col.key]}</div>
            </div>
              ...
              {filteredTasks
                ...
                .map((t) => (
                  <div key={t.id} style={{ background: 'rgba(3,6,16,0.9)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 8 }}>
                    ...
                    <div style={{ marginTop: 5, display: 'flex', gap: 6, flexWrap: 'wrap', fontSize: 10, fontFamily: 'Share Tech Mono, monospace' }}>
                      <span style={{ color: '#7f93b4', border: '1px solid rgba(127,147,180,0.35)', borderRadius: 999, padding: '2px 6px' }}>
                        PROJECT: {t.project}
                      </span>
                      <span
                        style={{
